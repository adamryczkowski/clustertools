...
test_that(paste0("Receiving object from ", remote_host), {
  ...
  a<-srv_loc$send_objects(job_name = "send", named_list_of_objects = list(a=10:14, b="second string"), timeout = 0)
  #debugonce(srv_loc$receive_objects)
  a<-srv_loc$receive_objects(object_names = "a" , job_name = "receive1", timeout=-1)
  ...
  a<-srv_loc$receive_objects(object_names = c("a","b") , job_name = "receive2", timeout=-1)
  ans<-a$peek_return_value(flag_wait_until_finished = TRUE)
  expect_equal(class(ans), 'environment')
